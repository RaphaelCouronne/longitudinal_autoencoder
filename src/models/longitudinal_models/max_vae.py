...
class MaxVAE(nn.Module):

    def __init__(self, data_info, latent_dimension, data_statistics, **kwargs):

        self.model_name = "max_vae"
        self.model_type = "max_vae"
        super(MaxVAE, self).__init__()
        ...
        self.rnn = nn.RNN(input_size=self.encoder.out_dim+1, hidden_size=self.encoder.out_dim)

        self.boost_mlp = nn.Sequential(
            nn.Linear(in_features=self.encoder.out_dim, out_features=self.latent_dimension+1),
            nn.LeakyReLU(),
            nn.Linear(in_features=self.latent_dimension + 1, out_features=self.latent_dimension+1)
        )

        self.mlp = MLP_variational(self.latent_dimension+1, self.latent_dimension+1)
